 ...
 * Bad address.
 *
 * Thrown by address-manipulation routines.
 */
class SocketBadAddress : public SocketError {
public:
    const char *what();
};
...
private:
    explicit SockAddress(EType type, unsigned char *data);
...
public:    
    ...
    void operator=(const SockAddress &other);
    ~SockAddress();

    bool isValid() const;

    /**
     * From a long in host byte order.
     */
    static SockAddress v4(unsigned int address);

    /**
     * From 4 individual bytes.
     */
    static SockAddress v4(unsigned char a, unsigned char b,
                          unsigned char c, unsigned char d);

    /**
     * From a char[4].
     */
    static SockAddress v4(unsigned char *address);

    /**
     * From a string in numbers-and-dots notation (e.g. "10.12.110.57").
     */
    static SockAddress v4(const char *address);

    /**
     * Returns the address as a single unsigned integer, in host byte order.
     *
     * @throws SocketBadAddress if this is not an IPv4 address.
     */
    unsigned int v4toUint() const throw(SocketBadAddress);
...
/**
 * Interface for something that can be waited for.
 *
 * This is used for SocketSet, which is a select() wrapper. Sockets can simply
 * pass their file descriptor to the registrar here.
 */
class Waitable {
 ...
 * A socket.
 *
 * Base class, especially useful for SocketSets.
 */
...
public:
     ...
     */
    static SockAddress resolve(
            const char *name,
            unsigned int types = SockAddress::ANY_TYPE);

    /**
     * Waitable implementation: register with a SocketSet.
     */
...
public:
     ...
     *
     * This should be called once to do whatever initialization is required
     * before using sockets on that platform.
     *
     * Currently, only Windows actually needs this, to startup WSA.
     */
    static void init() throw(SocketFatalError);

    /**
     * Frees the socket: it is free()d by this module, but the file descriptor
     * is not closed. Instead, it is returned and can be used through the
     * native system calls.
     */
...
/**
 * Small adapter passed to a Waitable so it can register its sockets.
 */
class SocketSetRegistrar {
...
public:
    ...
    /**
     * Declares that the Waitable should be notified if this socket changes.
     */
